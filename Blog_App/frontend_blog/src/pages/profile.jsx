      ...
      >
        ...
        <div className="flex items-center justify-between px-4">
          <div className="flex items-center gap-4">
            <div
              className={`w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold ${theme === "dark" ? "text-white bg-gray-400" : "text-black bg-gray-200"}`}
            >
            ...
            <div>
              <h2
                className={`text-xl font-semibold ${theme === "dark" ? "text-white/90" : "text-gray-800"}`}
              >
                {data?.username}
              </h2>
              <p className="text-sm text-gray-300">
                Joined: {data ? data?.joinDate : "01-01-2026"}
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-3 gap-4 text-center mt-6">
          ...
          />
          <Box
            count={data ? data.bookmarks?.length : 0}
            text1={"Bookmarks"}
            text2={"View"}
          />
...
function Box({ count, text1, text2 }) {
  ...
  return (
    ...
    >
      ...
      <p
        className={`text-xs ${theme === "dark" ? "text-gray-200" : "text-gray-500"}`}
      >
        {text1}
      </p>
      <p
        className={`mt-2 text-xs ${theme === "dark" ? "text-sky-400" : "text-gray-400"}`}
      >
        {text2}
      </p>
    </div>
