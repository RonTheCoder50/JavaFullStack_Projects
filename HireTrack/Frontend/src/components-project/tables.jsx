import {
  removeRecentAnalysisAPI,
  removeUserAPI,
  viewRecentAnalysisAPI,
} from "@/API";
...
export function HistoryTable({
  ...
  async function handleDeleteAnalysis(filename) {
    const permission = confirm("do you want to delete ?");
  ...
  return (
    <div className="w-full max-w-[85%] flex flex-col gap-6 mx-auto overflow-x-auto rounded-2xl border border-gray-200 shadow-sm mt-36">
      <div className="pt-4 flex justify-between px-6 items-center">
        <h1 className="text-sm sm:text-base md:text-lg font-bold tracking-wide">
          HISTORY TABLE
        </h1>
...
export function DashBoardHistoryFunc({ data, refresh }) {
  ...
  return (
    <div className="w-full max-w-[80%] flex flex-col gap-6 mx-auto overflow-x-auto rounded-2xl border border-gray-200 shadow-sm">
      <h2 className="pl-8 pt-4 text-base font-medium tracking-wide underline underline-offset-2">
        Recent Activity
      </h2>
      <table className="w-full min-w-[700px] border-collapse">
        <thead
        ...
        <tbody>
          {data?.map((user, index) => (
            ...
            >
              ...
              <td className="px-6 py-5 text-sm">#{user?.userId}</td>

              <td className="px-6 py-5 text-sm font-medium">
                {user?.username}
              </td>

              <td className="px-6 py-5 text-sm">{user?.dateOfJoining}</td>
            ...
            >
              ...
              <TableData
                value={
                  analysis?.time?.split("T")[0] +
                    " | " +
                    analysis?.time?.split("T")[1].substring(0, 8) || undefined
                }
              />
              ...
              <td className="px-6 py-5">
                <div className="flex items-center justify-center gap-3">
                  <button
                    onClick={() =>
                      handleViewFile(analysis?.uid, analysis?.filename)
                    }
                    className="px-4 py-2 rounded-lg bg-red-100 text-red-700 text-sm font-medium hover:bg-red-200 transition"
                  >
                    <Eye />
